...
from model_custom import CustomENet
...
output_dir = Path("D:\\Suhaib\\DalLake\\output(ENet-custom test)")
...
def save_predicted_images(model, val_loader, save_dir, target_size=(1280, 720)):
    ...
    with torch.no_grad():
        for i, (img, target, names) in enumerate(val_loader):
            ...
            img = img.cpu().numpy().transpose(0, 2, 3, 1) * np.array([0.229, 0.224, 0.225]) + np.array([0.485, 0.456, 0.406])
