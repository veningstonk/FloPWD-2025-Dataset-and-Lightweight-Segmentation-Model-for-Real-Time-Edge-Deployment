...
from custom_ablation import ENet, CustomENet, CustomENetNoSE, CustomENetNoDW, CustomENetNoAsym
...
models = [
    ('ENet', ENet),
    ('CustomENet', CustomENet),
    ('CustomENetNoSE', CustomENetNoSE),
    ('CustomENetNoDW', CustomENetNoDW),
    ('CustomENetNoAsym', CustomENetNoAsym)
]

# Training loop for each model
for model_name, model_class in models:
    print(f"\nTraining {model_name}...")
    output_dir = Path(f"D:\\Suhaib\\DalLake\\output_{model_name}")
    ...
    stats_path = output_dir / "model_stats.txt"
    ...
    model = model_class(in_channels=3, num_classes=2)
    ...
    # Save FLOPS and params to a text file
    with open(stats_path, 'w') as f:
        f.write(f"FLOPS: {flops / 1e9:.2f}\n")
        f.write(f"Parameters: {params / 1e6:.2f}\n")
    ...
    def compute_metrics(preds, target, num_classes=2):
        ...
        mIoU = (intersection / (union + 1e-10)).mean().item()
        return mPA, mIoU
    ...
    def save_predicted_images(model, val_loader, save_dir, target_size=(1280, 720)):
        ...
        with torch.no_grad():
            for i, (img, target, names) in enumerate(val_loader):
                ...
                img_np = img.cpu().numpy().transpose(0, 2, 3, 1) * np.array([0.229, 0.224, 0.225]) + np.array([0.485, 0.456, 0.406])
                img_np = (img_np * 255).astype(np.uint8)

                for j in range(img.shape[0]):
                    overlay = img_np[j].copy()
                    pred = preds[j].cpu().numpy()
                    plastic_mask = (pred == 1)
                    overlay[plastic_mask] = [255, 0, 0]
                    gt_plastic = (target[j].numpy() == 1)
                    overlay[gt_plastic] = [0, 255, 0]
                    Image.fromarray(overlay).resize(target_size, Image.NEAREST).save(save_dir / f"{names[j]}_overlay.png")
    ...
    def plot_metrics(metrics, epoch, plot_dir):
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(plot_dir / f"training_metrics_epoch_{epoch}.png")
        plt.close()

    def save_individual_metrics(metrics, save_dir):
        for metric_name in metrics.keys():
            ...
            df.to_csv(save_dir / f"{metric_name}.csv", index=False)
    ...
    try:
        for epoch in range(epochs):
            ...
            train_fps = len(train_data) / epoch_time if epoch_time > 0 else 0
            ...
            val_fps = len(val_data) / epoch_time if epoch_time > 0 else 0
            ...
            if avg_val_mIoU > best_val_mIoU:
                ...
                torch.save(model.state_dict(), model_save_path)
                print(f"Saved best model with validation mIoU: {best_val_mIoU:.4f}")
    ...
    print(f"Saved model stats to {stats_path}")
