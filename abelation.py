...
output_dir = Path("D:\\Suhaib\\DalLake\\ablation")
...
train_data = DalLake(root=dal_lake_dir, split='train', image_size=image_size)
val_data = DalLake(root=dal_lake_dir, split='val', image_size=image_size)
train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True)
val_loader = DataLoader(val_data, batch_size=batch_size, shuffle=False)

# Class weights
class_weights = [0.105, 2.0]
...
for config in ablation_configs:
    ...
    pred_save_dir = exp_dir / "predictions"
    csv_dir = exp_dir / "metrics"
    ...
    print(f"Saved metrics for {component}_{variation} to {csv_dir}")