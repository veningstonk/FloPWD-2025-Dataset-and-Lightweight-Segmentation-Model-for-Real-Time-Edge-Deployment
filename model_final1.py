...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, internal_channels=None, 
                 bottleneck_type='regular', dropout_prob=0.2, dilation=1, asymmetric=True):
        super(Bottleneck, self).__init__()
        internal_channels = out_channels // 8 if internal_channels is None else internal_channels
...
class ENet(nn.Module):
    def __init__(self, in_channels=3, num_classes=2):
        ...
        self.stage1 = nn.Sequential(
            Bottleneck(16, 64, bottleneck_type='down', dropout_prob=0.2),
            Bottleneck(64, 64, dropout_prob=0.2)
        )
        
        self.stage2 = nn.Sequential(
            Bottleneck(64, 128, bottleneck_type='down', dropout_prob=0.2),
            Bottleneck(128, 128, dilation=4, dropout_prob=0.2, asymmetric=True),
            Bottleneck(128, 128, dilation=8, dropout_prob=0.2, asymmetric=True)
        )
        
        self.stage3 = nn.Sequential(
            Bottleneck(128, 128, dilation=4, dropout_prob=0.2, asymmetric=True),
            Bottleneck(128, 128, dilation=8, dropout_prob=0.2, asymmetric=True),
            Bottleneck(128, 128, dilation=16, dropout_prob=0.2, asymmetric=True)
        )
        ...
        self.stage5 = nn.Sequential(
            ...
            nn.BatchNorm2d(16),
            nn.PReLU(16),
            Bottleneck(16, 16, dropout_prob=0.2)
        )
